...
function doPost(e) {
  ...
  try {
    ...
    var recaptchaResponse = e.parameters['g-recaptcha-response'];
    if (!validateRecaptcha(recaptchaResponse)) {
      Logger.log("reCAPTCHA validation failed");
      return ContentService
        .createTextOutput(JSON.stringify({"result":"error", "error": "reCAPTCHA validation failed"}))
        .setMimeType(ContentService.MimeType.JSON);
    }
    
    Logger.log("reCAPTCHA validation successful");
    record_data(e);
    ...
    if (sendEmailTo) {
      MailApp.sendEmail({
        ...
        subject: "Resume Request Received",
...
function getDataColumns(data) {
  return Object.keys(data).filter(function(column) {
    return !(column === 'formDataNameOrder' || column === 'formGoogleSheetName' || column === 'formGoogleSendEmail' || column === 'honeypot' || column === 'g-recaptcha-response');
  });
}
